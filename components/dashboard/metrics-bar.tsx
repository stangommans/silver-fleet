...
import { Landmark, TrendingUp, History, Activity, ArrowUpRight, ArrowDownRight, BarChart3, Coins } from "lucide-react";
...
export function MetricsBar({ 
  ...
  return (
    <div className="grid grid-cols-2 lg:grid-cols-5 gap-4">
      ...
      {/* Metric 5: Passive Income (Dividends + Lending) */}
      <Card className="bg-[#1A1D21] border-[#3f3f46] p-4 flex flex-col gap-3 relative overflow-hidden group hover:border-[#00E676] transition-colors rounded">
         <div className="flex justify-between items-start z-10">
            <span className="font-heading text-[10px] font-bold text-[#a1a1aa] uppercase tracking-widest">Passive Income</span>
            <Coins className="w-4 h-4 text-[#00E676]" />
         </div>
         <div className="flex flex-col mt-2 z-10">
            <span className="font-heading text-2xl lg:text-3xl font-bold text-[#00E676] tracking-tighter privacy-mask">€{(totalDividends + totalLending).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
            <div className="flex gap-2 text-[10px] font-mono text-[#a1a1aa] mt-1">
               <span title="Dividends">D: €{totalDividends.toFixed(2)}</span>
               <span title="Stock Lending">L: €{totalLending.toFixed(2)}</span>
            </div>
         </div>
         <div className="absolute -right-10 -bottom-10 w-32 h-32 bg-[#00E676]/5 rounded-full blur-2xl z-0"></div>
      </Card>
    </div>
  );
}
